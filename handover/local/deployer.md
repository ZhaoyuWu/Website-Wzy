...
| **PENDING (file added 2026-07-24, not yet applied)** | `entry-likes-persistence.sql` | `public.entry_likes` table + indexes + RLS (no policies; service-role only) | Not applied yet — user must run in Supabase SQL editor. Non-breaking while missing: backend detects the absent table once and falls back to in-memory like records. |
...
| 2026-07-24 | Maintenance rounds 1-2: Supabase-only backend + recovery + hardening (T-006-1/2, T-003-1) | `413b090` then follow-ups | Dead local-Postgres auth removed, comment throttle, SPA-404 fix (cleanUrls), security headers, runtime-config override removal, token refresh, entry_likes (code side), keep-alive workflow | `nanami-live-eo3dinq1f-...` (after two rejected candidates: `g4gvo0uv9` routes broken, prebuilt `6t6bzun44` had localhost runtime-config) |
| 2026-07-24 later | Maintenance round 3: direct uploads + lazy routes + apiFetch (T-004-1, T-001-1) | merge `9425f50` | Signed-URL direct-to-storage upload pipeline, lazy admin bundles (452→390 kB), central apiFetch with 401-refresh-retry | `nanami-live-kbz2pi9az-...` |
| 2026-07-24 evening | Maintenance round 4: signals modernization (T-006-3) | merges of `671a341`..`94924bc` era | admin/media pages then story-timeline: extracted templates + signals, zero manual CD; home-page template extraction | `nanami-live-alv1g8ryr-...`, `nanami-live-2mxu00fmc-...` |
| 2026-07-28 | Maintenance round 5: dead-code removal + portfolio (T-006-1 follow-up, T-006-4) | merge of `523232e` + `d71a53d` | StoryTimelineComponent deleted, broken #story anchor removed, showcase README + CI (`tests.yml`, first run green) + LICENSE + docs split, admin-page tests (frontend 64/64) | `nanami-live-amnqq6xj0-...` |

### Session 4 operational notes (2026-07-24/28)

- **Incident**: Supabase free project `pltveorkgsxfccyuwidk` had auto-paused after ~3 months idle — domain returned NXDOMAIN, every data endpoint failed with `fetch failed`. Predated this session's changes. User restored it via dashboard; same project ref, no config changes needed. Prevention: `keep-alive.yml` (10-min ping from `main`) + its failure e-mails as alerting.
- **Vercel routing regression**: the long-standing `cleanUrls: true` + negative-lookahead rewrite combination hard-404ed every direct SPA route under Vercel's current router. Diagnosed via local `vercel build` output inspection; fix is a plain `/(.*)` catch-all with `cleanUrls` removed. If routes 404 again after a Vercel platform change, start there.
