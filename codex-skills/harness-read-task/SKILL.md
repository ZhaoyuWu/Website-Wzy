---
name: harness-read-task
description: Read parent task and role branch task, then validate alignment. Use when user asks for /read-task, read-task workflow, or equivalent role-based execution.
---

# Read Task

**When to use:** At the start of generator$harness-evaluator session.
...
2. Detect role and read matching branch task:
   - producer -> `handover/tasks$harness-generator.md`
   - auditor -> `handover/tasks$harness-evaluator.md`
