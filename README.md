...
| 8 | `entry-likes-persistence.sql` | Adds `entry_likes` so per-viewer like records (dedupe, `likedByMe`, admin audit) survive backend restarts. Missing it is non-breaking: the backend detects the absent table and falls back to in-memory records. |
